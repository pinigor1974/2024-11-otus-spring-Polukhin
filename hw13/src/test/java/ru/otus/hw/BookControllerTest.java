...
class BookControllerTest {
    ...
    void setUp() {
        ...
        given(bookService.findById(1)).willReturn(dbBooks.stream().map(BookDto::fromDomainObject).toList().get(0));
    }
    ...
    @Test
    public void testAuthenticatedOnBookEndpoint() throws Exception {
        mvc.perform(get("/api/books/1")
                        .with(user("user").authorities(new SimpleGrantedAuthority("ROLE_USER"))))
                .andExpect(status().isOk())
                .andExpect(content().json(mapper.writeValueAsString(bookService.findById(1))));;
    }
